...
|--------|-------|-----------|
| Project statement coverage | **65.16 %** (was 46.61 %; 43.88 % before P1.1) | must not fall |
| Project line coverage | **67.79 %** (was 49.32 %; 46.51 % before P1.1) | must not fall |
| **Completion-path coverage** | `completions/caret.ts` **83.8 % lines / 100 % funcs**; `completions/flow.ts` **73.01 %**; `completions/table.ts` **87.07 %**; `completions/structure.ts` and `completions/preview-text.ts` **100 %** — all were 0 %. `main.ts`, `providers/*`, `typora-utils.ts` still 0 % | must rise |
| Suppression count | 25 across 12 files | must not rise |
| Repo-wide lint errors | **158** (was 6044 — P0.4 removed the CRLF noise; 190 → 161 as P1.1 moved code out of `main.ts`; 161 → 158 when `completions/service.ts`'s import block was put in its configured order) | must not rise; burn-down is its own task on its own branch |
...
| `derived=null` on the test doc | 0 at list items (live 12/12) and 0 inside a mapped table cell. **A fence caret still refuses, deliberately and earlier than matching**: its anchor lives in CodeMirror, so the approximate-match branch is unreachable today (defence for a future fence path, not the fence fix). A fence's rendered text also carries a token no markdown line has — live, a leading `x ` from a hidden widget — which whitespace-insensitive matching cannot absorb, so unrendered subtrees (form controls, hidden elements) are now excluded from every block read and a NO-MATCH logs the block HTML, so the next run can name any widget that survives | must stay 0 — live run still hand-verified until Phase 4 |
